package com.segib.web.rest;

import com.segib.config.KafkaProperties;
...
class FaceRecogKafkaResourceIT {
    ...
    void setup() {
        ...
        FaceRecogKafkaResource kafkaResource = new FaceRecogKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/face-recog-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/face-recog-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
